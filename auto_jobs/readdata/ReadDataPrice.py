...
from data_core.auto_jobs.readdata.cleaning import Sta, Pca, Lda
...
class ReadData:
    ...
    def query(self):
        ...
        cursor = Db.cursor()
        cursor.execute(
            "SELECT price,produce,data_month,province_code,amphur_code,tambon_code,export FROM scheduled_auto_price_job  where job_id = %s",
            (self.job_id,))
    ...
    def get_Data_load(self, model, path):
        ...
        if self.status == 0:
            self.X = pd.DataFrame(self.data, columns=['province_code', 'amphur_code', 'tambon_code',
                                                      'data_month']).to_numpy().astype('float')
        if self.status == 1:
            self.X = pd.DataFrame(self.data,
                                  columns=['province_code', 'amphur_code', 'tambon_code', 'data_month',
                                           'export']).to_numpy().astype('float')
        # ลดมิจิ
        X_sta, stapath = Sta.Sta_(self.X, PathFile.READFILE_MODEL_PRICE, 1).staUpdate(model["sta"], path)
        X_pca, pcapath = Pca.Pca_(X_sta, PathFile.READFILE_MODEL_PRICE, 1).pcaUpdate(model["pca"], path)
        X_lda, ldapath = Lda.Lda_(X_pca, self.z, PathFile.READFILE_MODEL_PRICE, 1).ldaUpdate(model["lda"], path)
        dataLoad: dict = {"x": X_lda, "z": self.z, "job_id": self.job_id, "stapath": stapath, "pcapath": pcapath,
                          "ldapath": ldapath}
        return dataLoad
