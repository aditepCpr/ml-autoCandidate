...
def error_rate(realData, predictData):
    ...
    for i in range(len(diffs_error_rate)):
        ...
        if b > 100:
            b = 100
        print("deff[%d],errors from %0.2f - %0.2f :: %d %% = %d , ratio: %d" % (i, a, b, i * 10, diffs_error_rate[i], ratio))
...
if __name__ == '__main__':
    ...
    for i in range(range_s):
        pds = PredictDataProductBigfarm.PredictData(
            np.array([X1[i, 0], X2[i, 0], X3[i, 0], X4[i, 0], X5[i, 0]]).astype(float), 1000)
        a = pds.predictData()
    ...
    pd_answer.to_csv(PathFile.READFILE_EXCEL + 'data__ln.csv', encoding='utf-8-sig')
    # print(answer)
