...
from readdata.ReadDataCost import ReadData
# โชว์ np.array เต็ม size
np.set_printoptions(threshold=sys.maxsize)
import matplotlib.pyplot as plt
import numpy as np
import PredictDataCost
...
def plotg():
    ...
    X_all = [['water', X1], ['disaster', X2], ['suitability', X3]]
    i = 1
...
if __name__ == '__main__':
    ...
    z = ReadData.getPlantMaintenance()
    plotg()
    answer = []
    data_list = []
    range_s = 20
    for i in range(range_s):
        pds = PredictDataCost.PredictData(
            np.array([X1[i, 0], X2[i, 0], X3[i, 0]]).astype(float), job_ids)
        a = pds.predictData()
        answer.append([z[i, 0], a])
        data_list.append([X1[i, 0], X2[i, 0], X3[i, 0],z[i, 0], a])
