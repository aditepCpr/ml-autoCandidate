...
class ReadData:
    ...
    def query(self):
        ...
        cursor = Db.cursor()
        cursor.execute(
            "SELECT data_id,job_id,idbigfarm,plangyear,water,disaster,suitability,plant_maintenance,plant_breed,plant_sale_price,produce FROM scheduled_bigfarm_data where job_id = %s ",
            (self.job_id,))
    ...
    def cleanData(self, breed, data):
        ...
        if breed == '0101':
            data = data[(data['plant_sale_price'] < 50) & (data['plant_sale_price'] > 0) & (data['produce'] < 1000) & (
                        data['produce'] > 200)]  # ข้าว
        if breed == '0201':
            data = data[
